...
const wordsCounter = document.querySelector('#wordsCounter');

const text = `В начале Х111 в. Центральной Азии возникло новое государство-Монгольская империя.
...
function keydownHandler(e) {
    ...
    for(let i of letters) {             
        if(e.key.toLowerCase() === i.dataset.letters.toLowerCase()) {
...
function keyupHandler(e) {
    ...
    for(let i of letters) {
        if(e.key.toLowerCase() === i.dataset.letters.toLowerCase()) {
            ...
            if(i.dataset.letters == 'Backspace' || i.dataset.letters =='Shift' || i.dataset.letters == 'Tab' || i.dataset.letters =='CAPSLOCK' || i.dataset.letters == 'Alt') {
                
            }
            else if(i.dataset.letters == 'Enter') {
                press('\n')
                console.log('ok')
            }
            else {
...
function press (letter) {
    ...
    if(string.length == party.currentPrintedIndex) {                    
        ...
        party.wordsCounter = party.strings[party.currentStringIndex].split(' ').reduce((countWord)=> {return countWord+=1;},0)
        wordsCounter.textContent = Math.round((60000 * party.wordsCounter) / party.timerCounter);
          
    } 
    
    if(letter == mustLetter) {
        party.currentPrintedIndex+=1;
        party.commonCounter++;
        ...
        party.errorCounter++;
        console.log( party.errorCounter)
    }
...
function viewUpdate () {
    ...
    input.value = party.strings[party.currentStringIndex].slice(0,party.currentPrintedIndex); 
